<div>
        ...
        @elseif(\App\Models\Follower::where('user_id', Auth::id())->where('target_id', $lastPost->users->id)->exists())
            <button wire:poll.1s wire:click="unfollow({{ $lastPost->users->id }})" class="px-3 py-2 bg-red-600 rounded-full">
                Ne plus suivre
            </button>
        @else
            <button wire:poll.1s wire:click="follow({{ $lastPost->users->id }})" class="px-3 py-2 bg-primary rounded-full">
                Suivre
            </button>
        @endif
