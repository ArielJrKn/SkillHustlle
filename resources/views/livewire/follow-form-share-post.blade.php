<div>
    ...
    @elseif(\App\Models\Follower::where('user_id', Auth::id())->where('target_id', $sharePost->users->id)->exists())
        <button wire:click="unfollow({{ $sharePost->users->id }})" class="px-3 py-2 bg-red-600 rounded-full">
            Ne plus suivre
        </button>
    @else
        <button wire:click="follow({{ $sharePost->users->id }})" class="px-3 py-2 bg-primary rounded-full">
            Suivre
        </button>
    @endif
</div>
