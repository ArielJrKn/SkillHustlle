...
<div wire:poll.1s class="relative flex flex-col w-full pt-2 rounded-full">
    <div class="flex items-center justify-around">
        <div class="searchBtnBack bg-gray-100 text-gray-900 dark:text-gray-100 dark:bg-dark dark:bg-opacity-30 px-6 py-1 rounded-full">
    ...
    @if(strlen($query) >= 2)
        @if(count($result['users'] ?? []) > 0 || count($result['posts'] ?? []) > 0)
            <div class="absolute top-12 z-50 bg-primary bg-opacity-20 text-black dark:text-white backdrop-filter backdrop-blur-2xl p-3 w-full mt-2 rounded-md shadow-lg max-h-96 overflow-y-auto">
