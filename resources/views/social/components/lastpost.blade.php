@if($lastPost->type === 'sharePost')
    <article class="post bg-primary bg-opacity-20 rounded-md p-6"  data-post-id="{{ $lastPost->id }}">
        ...
        @if($lastPost->type === 'sharePost')
            @php
                $sharePost = \App\Models\Post::where('id', $lastPost->post_id)->first();
            @endphp
        ...
        <div class="mt-4 text-xs text-primary foot flex items-center justify-between">
            <div class="icone flex items-center gap-1">
                <form class="like-form" action="{{route('likelastPost', $lastPost->id)}}" method="POST">
                    @csrf
                ...
                <i class="ri-chat-3-line comment bg-primary bg-opacity-20 px-4 py-2 rounded-md" data-post-id="{{ $lastPost->id }}"></i>
                <i class="ri-share-line bg-primary bg-opacity-20 px-4 py-2 rounded-md shareBtns" data-post-id="{{ $lastPost->id }}"></i>
            </div>

            <div class="info_foot flex gap-2">
                @include('social.likeCount')
                <h2 class="hidden lg:flex md:flex">܀</h2>
                <h2 class="hidden lg:flex md:flex">{{$lastPost->comments->where('type', 'post')->Count() }} comments</h2>
                <h2 class="hidden lg:flex md:flex">܀ </h2>
                <h2 class="hidden lg:flex md:flex">{{$lastPost->updated_at->diffForHumans()}}</h2>
            </div>
        </div>
        ...
        <div class="commentBox" id="commentBox-{{ $lastPost->id }}">
            <div class="pt-4 mt-4 border-t border-6 border-primary w-full h-96 overflow-y-auto">
                @auth
                    @php
                        $comments = $lastPost->comments->where('type', 'post');
                    @endphp
                        ...
                        @foreach($comments as $comment)
                            ...
                            @endif
                                    ...
                                    <div class="ml-3 flex mt-2 items-center gap-2 relative">
                                        ...
                                        @elseif(Auth::id() === $lastPost->user_id)
                                            <div class="flex justify-end">
                                                <form method="POST" action="{{route('social.destroyComment', $comment)}}">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        <div>Réponses {{$lastPost->comments->where('type', 'comment')->where('comment_id', $comment->id)->count()}}</div>
                                    </div>

                                    <!-- Réponses imbriquées (niveau 1) -->
                                    @php
                                        $replyComments = $lastPost->comments->where('type', 'comment')->where('comment_id', $comment->id);
                                    @endphp

                                    @foreach($replyComments as $replyComment)
                                    <div class="mt-6 space-y-4 pl-3 border-l border-gray-500">
...
@elseif($lastPost->type === 'simplePost')
    <article class="post bg-primary bg-opacity-20 rounded-md p-6" data-post-id="{{ $lastPost->id }}">
        <div class="flex items-center justify-between">
            <div class="flex align-center ">
                @auth
